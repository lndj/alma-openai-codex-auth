 ...
 */
// Maps model family → filename in `codex-rs/core/` of the openai/codex repo.
// The upstream repo renamed/removed several prompt files; entries below match
// what actually exists at HEAD as of late 2025. If a file ever disappears
// again, FALLBACK_PROMPT_FILE is used.
const FALLBACK_PROMPT_FILE = 'gpt_5_codex_prompt.md';
const PROMPT_FILES: Record<ModelFamily, string> = {
    'gpt-5.4': FALLBACK_PROMPT_FILE,         // no upstream file for 5.4 yet
    'gpt-5.3-codex': FALLBACK_PROMPT_FILE,   // upstream filename was removed
    'gpt-5.3': FALLBACK_PROMPT_FILE,         // ditto
...
async function getLatestReleaseTag(): Promise<string> {
    try {
        ...
        // 403 from anonymous rate limit is common — fall through to HTML fallback.
    } catch {
        // Network error — fall through to HTML fallback.
    }

    // Fallback: parse from HTML redirect (anonymous, no rate limit).
...
export async function getCodexInstructions(normalizedModel = 'gpt-5.1-codex'): Promise<string> {
    ...
    try {
        ...
        // If upstream renamed/removed our prompt file (404), retry with the
        // generic codex prompt. Avoids breaking on every codex-cli release.
        if (response.status === 404 && promptFile !== FALLBACK_PROMPT_FILE) {
            const fallbackUrl = `https://raw.githubusercontent.com/openai/codex/${latestTag}/codex-rs/core/${FALLBACK_PROMPT_FILE}`;
        ...
        if (existsSync(cacheFile)) {
            console.error(`[openai-codex-auth] Using stale cached ${modelFamily} instructions`);
            return readFileSync(cacheFile, 'utf8');
        }
