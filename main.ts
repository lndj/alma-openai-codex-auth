...
import { enableProxyFetch, proxyFetch, setProxyUrl } from './lib/proxy-fetch';
...
export async function activate(context: PluginContext): Promise<PluginActivation> {
    ...
    // here doesn't go through the host process's network stack.
    //
    // Activation runs BEFORE Alma's HTTP API server is listening, so the first
    // resolve attempt will usually fail with ECONNREFUSED. Enable with whatever
    // we get (may be null) and start a background poll that injects the proxy
    // URL via setProxyUrl as soon as the API server becomes reachable.
    const proxyUrl = await resolveProxyUrl(logger);
    enableProxyFetch(proxyUrl, logger);
    if (!proxyUrl) {
        void backgroundProxyPoll(logger);
    }
    ...
    const createCodexFetch = (): typeof globalThis.fetch => {
        return async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
            ...
            const response = await proxyFetch(codexUrl, {
                ...init,
                body,
                headers,
            });
            ...
            if (!response.ok) {
                ...
                // Surface the upstream error to the user so they see WHY it failed.
                // Without this, AI SDK only renders a generic "Bad Request" string.
                try {
                    const reqBody = typeof body === 'string' ? body : '';
                    let modelTried = '';
                    try { modelTried = (JSON.parse(reqBody) as { model?: string }).model ?? ''; } catch { /* noop */ }
                    ui.showError(
                        `Codex ${response.status} ${response.statusText}` +
                        (modelTried ? ` (model=${modelTried})` : '') +
...
const ALMA_API_HOST = '127.0.0.1';
const ALMA_API_PORT = 23001;
const SETTINGS_FETCH_TIMEOUT_MS = 3000;

interface AlmaProxyConfig {
    enabled?: boolean;
    type?: string;
    host?: string;
    port?: number;
...
async function resolveProxyUrl(logger: MinimalLogger): Promise<string | null> {
    ...
    if (envProxy && envProxy.trim().length > 0) {
        logger.info?.(`Using proxy from environment: ${envProxy.trim()}`);
        return envProxy.trim();
    }

    try {
        const settings = await fetchAlmaSettings() as { network?: { proxy?: AlmaProxyConfig } } | null;
        const p = settings?.network?.proxy;
        if (p && p.enabled && p.host && p.port) {
            const type = (p.type || 'http').toLowerCase();
            if (type === 'socks5') {
                logger.warn?.('Alma proxy is socks5, which this plugin does not support; falling back to direct.');
                return null;
            }
            ...
            logger.info?.(`Using proxy from Alma settings: ${p.host}:${p.port}`);
            return url;
        }
    } catch (err) {
        // Activation runs before Alma's HTTP API server is listening, so the
        // first attempt usually fails with ECONNREFUSED. backgroundProxyPoll
        // will retry until it succeeds.
        logger.warn?.(
            `Could not load proxy from Alma settings: ${err instanceof Error ? err.message : String(err)}`,
        );
    }
...
/**
 * Activation runs before Alma's HTTP API is up. Poll until we can reach it,
 * then push the resolved proxy URL into proxy-fetch's module state.
 */
async function backgroundProxyPoll(logger: MinimalLogger): Promise<void> {
    const delays = [500, 1000, 1500, 2000, 3000, 5000, 8000, 13000, 21000];
    for (const delay of delays) {
        ...
        try {
            ...
            if (p && p.enabled && p.host && p.port) {
                const type = (p.type || 'http').toLowerCase();
                if (type === 'socks5') {
                    logger.warn?.('Alma proxy is socks5, which this plugin does not support; staying in direct mode.');
                    return;
                }
                const auth = p.username
                    ? `${encodeURIComponent(p.username)}:${encodeURIComponent(p.password || '')}@`
                ...
                const url = `http://${auth}${p.host}:${p.port}`;
                setProxyUrl(url);
                logger.info?.(`Resolved proxy from Alma settings (poll +${delay}ms): ${p.host}:${p.port}`);
                return;
            }
            return;
        } catch {
            // Likely ECONNREFUSED — Alma API not up yet. Try again next iteration.
        }
    }
    logger.warn?.('Could not reach Alma settings API within 60s; staying in direct mode.');
}

function fetchAlmaSettings(): Promise<unknown> {
    // @ts-ignore
    ...
    return new Promise((resolve, reject) => {
        const req = http.request(
            {
                ...
                path: '/api/settings',
                method: 'GET',
                timeout: SETTINGS_FETCH_TIMEOUT_MS,
            },
            (res) => {
                const chunks: Buffer[] = [];
        ...
        req.on('timeout', () => {
            req.destroy(new Error('Alma settings request timed out'));
        });
        req.on('error', reject);
        req.end();
    });
